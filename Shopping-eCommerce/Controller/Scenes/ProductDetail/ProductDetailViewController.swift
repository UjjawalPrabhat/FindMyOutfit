        ...
        userRef.getDocument { document, error in
            if let document = document, document.exists {
                if let data = document.data(), let _ = data["\(productId)"] {
                    ...
                    DuplicateFuncs.alertMessage(title: "Caution", message: "The quantity of the product in the cart has been increased by 1.", viewController: self)
                } else {
                    userRef.updateData(["\(productId)" : quantity])
                    DuplicateFuncs.alertMessage(title: "Success", message: "The product has been successfully added to the cart.", viewController: self)
                }
            } else {
