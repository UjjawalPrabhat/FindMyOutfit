...
class RegisterViewController: UIViewController {
                ...
                Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
                    if let error = error {
                        ...
                        changeRequest?.commitChanges(completion: { error in
                            if let error = error {
                                DuplicateFuncs.alertMessage(title: "FIRESTORE ERROR", message: error.localizedDescription, viewController: self)
                            }
                        })
                        ...
                        DuplicateFuncs.alertMessageWithHandler(title: "Verify your email", message: "Verification mail sent", viewController: self) {
                            self.performSegue(withIdentifier: K.Segues.registerToLogin, sender: self)
                        }
                    }
                }
            } else {
                DuplicateFuncs.alertMessage(title: "ERROR", message: "Passwords do not match!", viewController: self)
            }
        }
    }
    ...
    func sendVerificationMail() {
        if authUser != nil && !authUser!.isEmailVerified {
            ...
            DuplicateFuncs.alertMessage(title: "ERROR", message: "e-mail already verified", viewController: self)
        }
    }
