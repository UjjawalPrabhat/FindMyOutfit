...
class ProfileViewController: UIViewController {
    ...
    func signOut() {
        do {
            ...
            DuplicateFuncs.alertMessageWithHandler(title: "Success", message: "Logged out", viewController: self) {
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let viewController = storyboard.instantiateViewController(withIdentifier: K.StoryboardID.welcomeVC)
    ...
    func uploadProfilePhotoToFirebase() {
        ...
        let uploadTask = storageRef.putData(imageData) { (metadata, error) in
            if error != nil {
                DuplicateFuncs.alertMessage(title: "ERROR", message: "Error uploading profile photo: \(String(describing: error?.localizedDescription))", viewController: self)
                return
            }
            storageRef.downloadURL { (url, error) in
                if error != nil {
                    DuplicateFuncs.alertMessage(title: "ERROR", message: "Error getting download URL for profile photo", viewController: self)
                    return
                }
                ...
                self.database.collection("users").document(self.currentUserUid!).collection("userInfo").document(self.currentUserUid!).setData([
                    ...
                    if error != nil {
                        ...
                        DuplicateFuncs.alertMessage(title: "ERROR", message: "Failed to save profile photo.", viewController: self)
                        return
                    }
                    DuplicateFuncs.alertMessage(title: "Success", message: "Profile photo successfully saved.", viewController: self)
                }
            }
        }
        uploadTask.resume()
    ...
    func fetchProfilePhotoFromFirebase() {
        database.collection("users").document(currentUserUid!).collection("userInfo").document(currentUserUid!).getDocument { document, error in
            if error != nil {
                DuplicateFuncs.alertMessage(title: "ERROR", message: "Failed to fetch profile photo: \(String(describing: error?.localizedDescription))", viewController: self)
                return
            }
    ...
    func fetchUserNameAndEmail() {
        database.collection("users").document(currentUserUid!).collection("userInfo").document(currentUserUid!).getDocument { document, error in
            if error != nil {
                DuplicateFuncs.alertMessage(title: "ERROR", message: "Failed to fetch username and email", viewController: self)
            } else {
                guard let document = document, document.exists else { return }
