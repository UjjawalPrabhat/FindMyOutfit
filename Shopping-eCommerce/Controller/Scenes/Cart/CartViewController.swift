...
class CartViewController: UIViewController {
    ...
    @IBAction func checkoutButtonPressed(_ sender: UIButton) {
        ...
        if CartViewController.cartItems.count == 0 {
            DuplicateFuncs.alertMessage(title: "ERROR", message: "Your cart is empty", viewController: self)
        } else {
                ...
                if let productId = product.id {
                    userRef.updateData([
                        ...
                        if let error = error {
                            ...
                            DuplicateFuncs.alertMessage(title: "Order Success", message: "Your order has been placed", viewController: self)
                        }
                    }
                }
            }
            ...
            if increment {
                if updatedQuantity < 10 {
                    userRef.updateData(["\(productId)": FieldValue.increment(Int64(1))]) { error in
                        if let _ = error {
                            DuplicateFuncs.alertMessage(title: "ERROR", message: "Product quantity could not be changed", viewController: self)
                        }
                    }
                }
            } else {
...
extension CartViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let product = CartViewController.cartItems[indexPath.row]
        ...
        cell.productQuantity.text = "\(String(describing: product.quantityCount!))"
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            docRef.updateData(["\(String(describing: productId!))": FieldValue.delete()]) { error in
                if let error = error {
                    DuplicateFuncs.alertMessage(title: "Error", message: "Product could not be removed", viewController: self)
                    print("Product deletion error: \(error.localizedDescription)")
                } else {
                    DuplicateFuncs.alertMessage(title: "Success", message: "Product removed from cart", viewController: self)
                }
            }
        }
    }
